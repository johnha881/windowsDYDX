...
def open_positions(client):
    ...
    """"
    MANGE FINDING TRIGGERS FOR TRADE ENTRY
    STORE TRADES FOR MANGING LATER ON THE EXIT FUNCTION
    """
        ...
        if len(series_1) > 0 and len(series_1) == len(series_2):
            ...
            if abs(z_score) >= ZSCORE_THRESH:
                ...
                if not is_base_open and not is_quote_open:
                    ...
                    # Ensure size
                    # keep in mind min order size, can increase if got lots of money
